...
from utils import load_settings, delete_all_messages, delete_guild_data, log_with_guild_info
...
async def check_streams():
    for guild_id, guild_settings in settings.get('guilds', {}).items():
        ...
        if category_name:
            ...
            log_with_guild_info(guild_id, 'warning', f"No category_name set for guild ID {guild_id}")
...
async def on_ready():
    ...
    log_with_guild_info(None, 'info', f'Logged in as {bot.user}')
    ...
    for guild_id, guild_settings in settings.get('guilds', {}).items():
        ...
        if channel_id and category_name:
            ...
            if not check_streams.is_running():
                check_streams.start()
            log_with_guild_info(guild_id, 'info', f"Starting stream check for guild ID {guild_id} with category {category_name}")
        else:
            if not channel_id:
                log_with_guild_info(guild_id, 'error', f"Missing channel_id for guild ID {guild_id}")
            if not category_name:
                log_with_guild_info(guild_id, 'error', f"Missing category_name for guild ID {guild_id}")
...
async def on_guild_remove(guild):
    ...
    log_with_guild_info(guild_id, 'info', f"Bot was removed from guild: {guild.name} (ID: {guild_id})")
